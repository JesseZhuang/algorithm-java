...
public class MedianTwoSortedArrays {
    ...
    // binary search, O(log(min(m,n)) time, O(1) space, 2 ms 100%, 46.2 Mb 92.36%.
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        int N1 = nums1.length, N2 = nums2.length; // {1,2,3,4,5}, {1,1,1,1}; 2N1+2N2+2 positions
        if (N1 < N2) return findMedianSortedArrays(nums2, nums1);
        int lo = 0, hi = 2 * N2; // binary search on nums2, mid1 has to move accordingly mid1+mid2 == N1+N2
        while (lo <= hi) {
            int mid2 = lo + (hi - lo) / 2; // practical: on the shorter array, all positions are possible median cuts
            int mid1 = N1 + N2 - mid2;  // mid1+mid2 == N1+N2 for median definition
            ...
            // L1 <= R1 && L1 <= R2(sorted,omit) && L2 <= R1 && L2 <= R2(sorted,omit) -> L1 <= R2 and L2 <= R1
