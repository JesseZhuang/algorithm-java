...
public class RunLength {
    ...
    private static final int LG_R = 8; // 8 bit, run length [0,255]
    ...
    public static void compress() {
        char run = 0; // run length 0
        boolean old = false; // whether the value of the next bit is the same as before, assume start with 0
        while (!BinaryStdIn.isEmpty()) {
            boolean b = BinaryStdIn.readBoolean();
            if (b != old) { // start new run
                BinaryStdOut.write(run, LG_R); // write as 8 bit char
                run = 1;
                old = !old; // flip 0->1 or 1->0
            } else {
                if (run == R - 1) { // run length is 255 max
                    BinaryStdOut.write(run, LG_R);
                    run = 0;
                    BinaryStdOut.write(run, LG_R);
                }
                run++; // same value as old bit, increment run length
            }
        }
        BinaryStdOut.write(run, LG_R);
        BinaryStdOut.close();
